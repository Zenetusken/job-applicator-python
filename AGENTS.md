...
- **Indeed runs HEADED + ephemeral profile (Cloudflare).** Indeed is fronted by a Cloudflare *managed JS challenge* that blocks headless Chrome (empirically 100%; the wall is NOT TLS/JA3 — bundled Chromium's JA4 matches real Chrome — and NOT rate-limit). The fix needs no special engine: run headed from a clean profile and the existing Playwright+stealth stack passes (returns real jobs). `cli._make_browser(site, settings)` encodes this — Indeed → `BrowserManager(cfg(headless=False), ephemeral_profile=True, virtual_display=True)`; LinkedIn → unchanged headless persistent. `virtual_display` runs headed windowless via Xvfb (`pyvirtualdisplay`, optional `[indeed]` extra; falls back to ambient `$DISPLAY` or `xvfb-run`). `--headed` shows a real window (no virtual display). `IndeedScraper.login()` returns False; `scrape()` still loads any imported `cf_clearance` as a warm start but passes cold too; raises `ScraperError` on a challenge. See `docs/compose/reports/2026-06-15-indeed-cloudflare-research.md` for the full matrix.
- **Indeed region is derived from the host timezone.** `target.indeed_domain` empty (default) → `region.detect_indeed_domain()` maps the IANA timezone → ISO country via `/usr/share/zoneinfo/zone1970.tab` → `<cc>.indeed.com` (e.g. America/Toronto → `ca.indeed.com`; US → `www`; GB → `uk`). Uses the *timezone*, not the locale (often `en_US` even in Canada). Indeed does NOT reliably redirect www→region by IP, so picking the host up front matters. Pin `target.indeed_domain` to override.
