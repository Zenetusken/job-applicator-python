...
├── scrapers/           # base.py (ABC + BrowserPolicy/browser_policy) → linkedin.py, indeed.py (both live; Indeed selectors tuned 2026-06-15)
...
- **Indeed runs HEADED + ephemeral profile (Cloudflare); the policy lives on the board.** Indeed is fronted by a Cloudflare *managed JS challenge* that blocks headless Chrome (empirically 100%; the wall is NOT TLS/JA3 — bundled Chromium's JA4 matches real Chrome — and NOT rate-limit). The fix needs no special engine: run headed from a clean profile and the existing Playwright+stealth stack passes (returns real jobs). A board declares its browser needs via `BaseScraper.browser_policy() -> BrowserPolicy` (`scrapers/base.py`); `IndeedScraper` overrides it to `BrowserPolicy(headed=True, ephemeral_profile=True, virtual_display=True)`, LinkedIn keeps the default (headless, shared persistent profile). `cli._make_browser(site, settings)` READS that policy (no `if site == "indeed"` special-case) and builds the `BrowserManager` — `virtual_display = policy.virtual_display and settings.browser.headless`, so `--headed` shows a real window instead of a virtual one. `_make_browser` resolves the board via `_scraper_class(site)`, which validates the site (raises `typer.Exit`) BEFORE any browser launches. `virtual_display` runs headed windowless via Xvfb (`pyvirtualdisplay`, optional `[indeed]` extra; falls back to ambient `$DISPLAY` or `xvfb-run`). `IndeedScraper.login()` returns False; `scrape()` warns if handed a headless browser (`BrowserManager.headless`), loads any imported `cf_clearance` as a best-effort warm start (NOT required — passes cold), and raises `ScraperError` on a challenge. Adding a 3rd headed board = override `browser_policy()` on its scraper, nothing in the CLI. Full matrix: `docs/compose/reports/2026-06-15-indeed-cloudflare-research.md`. (Indeed `search`/`batch` are validated; Indeed `apply` is wired through the same path but unvalidated.)
- **Indeed region is derived from the host timezone, against an allowlist.** `target.indeed_domain` empty (default) → `region.detect_indeed_domain()` maps the IANA timezone → ISO country via `/usr/share/zoneinfo/zone1970.tab`, then to `<cc>.indeed.com` ONLY if that country is in `_INDEED_COUNTRIES` (the set Indeed actually serves; `gb`→`uk` override). Anything else — the US, or a country with no Indeed site (Bahamas, Iceland, …) — falls back to `www.indeed.com`, so a dead `<cc>.indeed.com` (→ NavigationError) is never emitted. Uses the *timezone*, not the locale (often `en_US` even in Canada). `IndeedScraper._base` caches the auto-detected origin on the instance (computed once, not per result card). Pin `target.indeed_domain` to override.
- **Untyped deps get a local stub, not an ignore.** `pyvirtualdisplay` is typed via `stubs/pyvirtualdisplay/__init__.pyi` (on `mypy_path = "stubs"`); paddleocr/fitz/playwright_stealth/browser_cookie3 use `[[tool.mypy.overrides]] ignore_missing_imports`. Prefer a stub when you use specific attributes.
