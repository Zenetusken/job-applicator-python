...
# Tests — ~1366 fast unit tests (the green gate); ~1424 total = ~1366 unit + 23 integration + 35 live
...
- Tests are auto-marked by location (`tests/conftest.py`): `pytest -m unit` / `-m live` /
  `-m integration` all work. Unit suite (`pytest -m unit`, ~1366) is fast — no browser/GPU; the green
  gate.
