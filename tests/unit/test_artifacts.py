...
async def test_write_tailored_pdf_renders_to_spec_path_and_writes_meta(
    tmp_path: Path,
) -> None:
    ...
    with patch.object(
        ...
        path = await write_tailored_pdf(output_dir, tailored, settings, when=WHEN)
...
async def test_write_cover_letter_pdf_renders_to_spec_path_and_writes_meta(
    tmp_path: Path,
) -> None:
    output_dir = tmp_path / "pdfs"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    with patch.object(
        ...
        path = await write_cover_letter_pdf(output_dir, result, settings, when=WHEN)
...
async def test_write_tailored_pdf_passes_category_to_renderer(tmp_path: Path) -> None:
    ...
    with patch.object(
        ...
        await write_tailored_pdf(output_dir, tailored, settings, category="tech-support", when=WHEN)
...
async def test_write_cover_letter_pdf_passes_category_to_renderer(
    ...
    with patch.object(
        ...
        await write_cover_letter_pdf(
            output_dir, result, settings, category="tech-support", when=WHEN
        )
...
async def test_write_cover_letter_pdf_wraps_render_failure(tmp_path: Path) -> None:
    ...
    with patch.object(
        ...
        with pytest.raises(PDFRenderError, match="Failed to render cover-letter PDF"):
            await write_cover_letter_pdf(output_dir, result, settings, when=WHEN)
