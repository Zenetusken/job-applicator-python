...
def _drive_cover_letter(
    ...
    *,
    ...
    cl_generator.generate_verified_with_overlay = AsyncMock(
        return_value=(cover_letter_text, cover_letter_overlay())
    )
...
def test_apply_dry_run_generates_cover_letter() -> None:
    ...
    assert result.exit_code == 0, result.output
    cl_generator.generate_verified_with_overlay.assert_awaited_once()
...
def test_apply_dry_run_no_cover_letter_flag_skips_generation() -> None:
    ...
    assert result.exit_code == 0, result.output
    cl_generator.generate_verified_with_overlay.assert_not_awaited()
    applicator.apply.assert_awaited_once()
    call = applicator.apply.await_args
    cover_letter = call.args[1] if len(call.args) > 1 else call.kwargs.get("cover_letter")
...
def test_apply_style_guide_flows_to_generator() -> None:
    ...
    cl_generator.generate_verified_with_overlay = AsyncMock(
        return_value=("Styled cover letter", cover_letter_overlay())
    )
    ...
    cl_generator.generate_verified_with_overlay.assert_awaited_once()
    call = cl_generator.generate_verified_with_overlay.await_args
