...
path is the committed smoke driver **`.claude/skills/run-job-applicator/driver.sh`**,
...
```bash
bash .claude/skills/run-job-applicator/driver.sh           # CORE + LIVE (LIVE auto-skips if vLLM is down)
bash .claude/skills/run-job-applicator/driver.sh --core    # offline only
bash .claude/skills/run-job-applicator/driver.sh --live    # live only
```
