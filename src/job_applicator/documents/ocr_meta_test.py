...
_VENV_CLI = PROJECT_ROOT / ".venv" / "bin" / "job-applicator"
CLI = _VENV_CLI if _VENV_CLI.exists() else Path(shutil.which("job-applicator") or str(_VENV_CLI))
...
def run_cli(args: list[str], timeout: int = 120) -> tuple[subprocess.CompletedProcess[str], float]:
    """Run the CLI and return the completed process and elapsed time."""
    cmd = [str(CLI), *args]
